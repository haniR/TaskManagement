 ...
 * @author ASUS
 */
public class PriorityChartMapper implements RowMapper<Chart> {

    @Override
    public Chart mapRow(ResultSet row, int i) throws SQLException {
        Chart priorityChart = new Chart();
        priorityChart.setName(row.getString("priority"));
        priorityChart.setCounter(row.getInt("counter"));
